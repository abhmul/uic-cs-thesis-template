...
\begin{lemma}
    \((M_{n})_{n \geq 1}\) is a martingale adapted to filtration \((\mathcal{F}_{n})_{n \geq 1}\) iff there exists a martingale difference sequence $d: \Omega \to \mathbb{R}^{\mathbb{N}}$  adapted to the same filtration and \(c \in \mathbb{R}\) so that
...
\begin{proof}
    ...
    \((\Leftarrow)\): We are given difference sequence \((d_{n})_{n \geq 1}\) and constant \(c \in  \mathbb{R}\). First checking for \(n \in \mathbb{N}\) that \(M_{n} \in L^{1}(\mathcal{B})\), consider
    \begin{align*}
